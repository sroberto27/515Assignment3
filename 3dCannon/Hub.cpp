...
Hub::Hub() {
	initHub(0, 0, 0, 0, 0, 0, 0, 0);
}
Hub::Hub(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initHub(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Hub::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Hub::setVertices(int num) {
	numVertices = num;
}


void Hub::readPointFileHub(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void Hub::initHub(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileHub("Hub.txt");
...
int Hub::getNumVertices() { return numVertices; }
int Hub::getNumIndices() { return numIndices; }
std::vector<int> Hub::getIndices() { return indices; }
std::vector<glm::vec3> Hub::getVertices() { return vertices; }
std::vector<glm::vec2> Hub::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Hub::getNormals() { return normals; }
std::vector<glm::vec3> Hub::getStangents() { return sTangents; }
std::vector<glm::vec3> Hub::getTtangents() { return tTangents; }