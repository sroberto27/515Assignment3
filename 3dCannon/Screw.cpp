...
Screw::Screw() {
	initScrew(0, 0, 0, 0, 0, 0, 0, 0);
}
Screw::Screw(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initScrew(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Screw::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Screw::setVertices(int num) {
	numVertices = num;
}


void Screw::readPointFileScrew(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void Screw::initScrew(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileScrew("Screw.txt");
...
int Screw::getNumVertices() { return numVertices; }
int Screw::getNumIndices() { return numIndices; }
std::vector<int> Screw::getIndices() { return indices; }
std::vector<glm::vec3> Screw::getVertices() { return vertices; }
std::vector<glm::vec2> Screw::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Screw::getNormals() { return normals; }
std::vector<glm::vec3> Screw::getStangents() { return sTangents; }
std::vector<glm::vec3> Screw::getTtangents() { return tTangents; }