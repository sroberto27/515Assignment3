...
Cheek::Cheek() {
	initCheek(0, 0, 0, 0, 0, 0, 0, 0);
}
Cheek::Cheek(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initCheek(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Cheek::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Cheek::setVertices(int num) {
	numVertices = num;
}


void Cheek::readPointFileCheek(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void Cheek::initCheek(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileCheek("Cheek.txt");
...
int Cheek::getNumVertices() { return numVertices; }
int Cheek::getNumIndices() { return numIndices; }
std::vector<int> Cheek::getIndices() { return indices; }
std::vector<glm::vec3> Cheek::getVertices() { return vertices; }
std::vector<glm::vec2> Cheek::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Cheek::getNormals() { return normals; }
std::vector<glm::vec3> Cheek::getStangents() { return sTangents; }
std::vector<glm::vec3> Cheek::getTtangents() { return tTangents; }