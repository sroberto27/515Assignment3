...
Trunnion::Trunnion() {
	initTrunnion(0, 0, 0, 0, 0, 0, 0, 0);
}
Trunnion::Trunnion(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initTrunnion(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Trunnion::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Trunnion::setVertices(int num) {
	numVertices = num;
}


void Trunnion::readPointFileTrunnion(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void Trunnion::initTrunnion(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileTrunnion("Trunnion.txt");
...
int Trunnion::getNumVertices() { return numVertices; }
int Trunnion::getNumIndices() { return numIndices; }
std::vector<int> Trunnion::getIndices() { return indices; }
std::vector<glm::vec3> Trunnion::getVertices() { return vertices; }
std::vector<glm::vec2> Trunnion::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Trunnion::getNormals() { return normals; }
std::vector<glm::vec3> Trunnion::getStangents() { return sTangents; }
std::vector<glm::vec3> Trunnion::getTtangents() { return tTangents; }