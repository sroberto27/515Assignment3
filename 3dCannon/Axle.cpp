...
Axle::Axle() {
	initAxle(0, 0, 0, 0, 0, 0, 0, 0);
}
Axle::Axle(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initAxle(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Axle::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Axle::setVertices(int num) {
	numVertices = num;
}


void Axle::readPointFileAxle(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void Axle::initAxle(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileAxle("Axle.txt");
...
int Axle::getNumVertices() { return numVertices; }
int Axle::getNumIndices() { return numIndices; }
std::vector<int> Axle::getIndices() { return indices; }
std::vector<glm::vec3> Axle::getVertices() { return vertices; }
std::vector<glm::vec2> Axle::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Axle::getNormals() { return normals; }
std::vector<glm::vec3> Axle::getStangents() { return sTangents; }
std::vector<glm::vec3> Axle::getTtangents() { return tTangents; }