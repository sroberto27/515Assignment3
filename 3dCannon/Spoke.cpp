...
Spoke::Spoke() {
	initSpoke(0, 0, 0, 0, 0, 0, 0, 0);
}
Spoke::Spoke(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initSpoke(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Spoke::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Spoke::setVertices(int num) {
	numVertices = num;
}


void Spoke::readPointFileSpoke(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
	...
	for (size_t i = 0; i < verticesTemp.size(); i++)
	{
		...
		float amt = toRadians(i*360.0f / verticesTemp.size());
...
void Spoke::initSpoke(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileSpoke("Spoke.txt");
...
int Spoke::getNumVertices() { return numVertices; }
int Spoke::getNumIndices() { return numIndices; }
std::vector<int> Spoke::getIndices() { return indices; }
std::vector<glm::vec3> Spoke::getVertices() { return vertices; }
std::vector<glm::vec2> Spoke::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Spoke::getNormals() { return normals; }
std::vector<glm::vec3> Spoke::getStangents() { return sTangents; }
std::vector<glm::vec3> Spoke::getTtangents() { return tTangents; }