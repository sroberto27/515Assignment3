...
void Arrow::readPointFileArrow(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void Arrow::initArrow(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	...
	for (int ring = 1; ring < prec; ring++) {
		for (int i = 0; i < verticesTemp.size(); i++) {
			...
			vertices[ring+ i] = glm::vec3(rMat * glm::vec4(vertices[i], 1.0f));

			texCoords[ring + i] = glm::vec2((float)ring*2.0f / (float)prec, texCoords[i].t);
			...
			sTangents[ring + i] = glm::vec3(rMat * glm::vec4(sTangents[i], 1.0f));

			rMat = glm::rotate(glm::mat4(1.0f), amt, glm::vec3(1.0f, 0.0f, 0.0f));
			tTangents[ring + i] = glm::vec3(rMat * glm::vec4(tTangents[i], 1.0f));

			rMat = glm::rotate(glm::mat4(1.0f), amt, glm::vec3(1.0f, 0.0f, 0.0f));
			normals[ring+ i] = glm::vec3(rMat * glm::vec4(normals[i], 1.0f));
			*/
			float amt = (float)toRadians((float)ring * 360.0f / (prec));
